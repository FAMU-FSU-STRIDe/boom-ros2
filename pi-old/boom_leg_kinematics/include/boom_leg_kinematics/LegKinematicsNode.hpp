#ifndef BOOM_LEG_KINEMATICS_NODE_HPP_
#define BOOM_LEG_KINEMATICS_NODE_HPP_
...
#include <boom_interfaces/msg/leg_command_array.hpp>
#include <boom_interfaces/srv/configure_legs.hpp>
#include <boom_interfaces/msg/leg_info_array.hpp>
...
using namespace boom_interfaces::srv;
...
public:
    ...
    : rclcpp::Node("leg_kinematics") {
        ...
        this->cmd_sub_ = this->create_subscription<LegCommandArray>("/boom/legs/cmd", 10,
            std::bind(&LegKinematicsNode::cmd_legs_callback_, this, std::placeholders::_1));
        this->cmd_pub_ = this->create_publisher<ODriveCommandArray>("/boom/motors/cmd", 10);

        this->conf_srv_ = this->create_service<ConfigureLegs>("/boom/legs/conf",
            std::bind(&LegKinematicsNode::conf_legs_callback_, this, std::placeholders::_1, std::placeholders::_2));

        this->info_sub_ = this->create_subscription<ODriveInfoArray>("/boom/motors/info", 10,
            std::bind(&LegKinematicsNode::get_info_callback_, this, std::placeholders::_1));
        this->info_pub_ = this->create_publisher<LegInfoArray>("/boom/legs/info", 10);
...
private:
    ...
    void conf_legs_callback_(const boom_interfaces::srv::ConfigureLegs::Request::SharedPtr request,
                                const boom_interfaces::srv::ConfigureLegs::Response::SharedPtr response) {
        (void) response;
