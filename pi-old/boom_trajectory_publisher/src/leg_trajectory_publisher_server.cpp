...
#include <boom_interfaces/action/run_leg_trajectory.hpp>
#include <boom_interfaces/msg/o_drive_info_array.hpp>

#include <boom_trajectory_publisher/visibility_control.h>

namespace boom {

using namespace boom_interfaces::msg;

class LegTrajectoryPublisherServer : public rclcpp::Node {

public:

    using RunLegTrajectory = boom_interfaces::action::RunLegTrajectory;
    using GoalHandle = rclcpp_action::ServerGoalHandle<RunLegTrajectory>;

    explicit LegTrajectoryPublisherServer(const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
    : rclcpp::Node("trajectory_publisher", options), is_running_(false), is_in_fault_state_(false), fault_(0) {
        ...
        this->action_server_ = rclcpp_action::create_server<RunLegTrajectory>(this, "/boom/legs/run_trajectory",
            std::bind(&LegTrajectoryPublisherServer::handle_goal_, this, _1, _2),
            std::bind(&LegTrajectoryPublisherServer::handle_cancel_, this, _1),
            std::bind(&LegTrajectoryPublisherServer::handle_accepted_, this, _1));

        this->motor_info_sub_ = this->create_subscription<ODriveInfoArray>("/boom/motors/info", 10,
            std::bind(&LegTrajectoryPublisherServer::motor_info_callback_, this, _1));

        this->leg_cmd_pub_ = this->create_publisher<LegCommandArray>("/boom/legs/cmd", 10);

        using namespace std::chrono_literals;
        this->feedback_timer_ = this->create_wall_timer(20ms, std::bind(&LegTrajectoryPublisherServer::feedback_callback_, this));
...
private:
    ...
    void handle_accepted_(const std::shared_ptr<GoalHandle> goal_handle) {
        ...
        std::thread{std::bind(&LegTrajectoryPublisherServer::execute_, this)}.detach();
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(boom::LegTrajectoryPublisherServer)